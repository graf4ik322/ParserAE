...
## 🐛 BOT-004: Тайм-ауты в функциях квиза и вопросов о парфюмах из-за слишком больших промптов
...
### **Description:**
После изменений в форматировании промпты стали слишком большими из-за передачи ВСЕХ парфюмов без ограничений, что приводит к тайм-аутам в функциях квиза и вопросов о парфюмах.
...
- Промпты должны быть оптимизированы по размеру
- Не должно быть тайм-аутов

### **Root Cause:**
В файле `ai/prompts.py` промпты создаются с ВСЕМИ парфюмами без ограничений:
- `create_perfume_question_prompt()` передает все парфюмы в промпт
- `create_quiz_results_prompt()` также передает все подходящие парфюмы
- Размер промпта превышает лимиты API

### **Affected Files:**
- `ai/prompts.py` - методы создания промптов
...
### **Proposed Fix:**
Ограничить размер промптов:

```python
@staticmethod
def create_perfume_question_prompt(user_question: str, perfumes_data: List[Dict[str, Any]]) -> str:
    """Создает промпт для вопроса о парфюмах с ОГРАНИЧЕНИЕМ размера"""
    
    # Ограничиваем количество парфюмов для промпта
    MAX_PERFUMES_IN_PROMPT = 200  # Максимум 200 парфюмов в промпте
    
    # Формируем ОГРАНИЧЕННЫЙ список парфюмов
    perfumes_list = []
    factory_analysis = {}
    
    # Берем только первые MAX_PERFUMES_IN_PROMPT парфюмов
    limited_perfumes = perfumes_data[:MAX_PERFUMES_IN_PROMPT]
    ...
    # Ограниченный список парфюмов
    all_perfumes = perfumes_list
    
    # Создаем сводку по фабрикам
    factory_summary = []
    for factory, data in factory_analysis.items():
    ...
    # Добавляем информацию о том, что список ограничен
    if len(perfumes_data) > MAX_PERFUMES_IN_PROMPT:
        factory_summary.append(f"- Показано {MAX_PERFUMES_IN_PROMPT} из {len(perfumes_data)} ароматов")
...
### **Testing:**
1. Запустить квиз - должен работать без тайм-аутов
2. Задать вопрос о парфюмах - должен получить ответ
3. Проверить логи - не должно быть тайм-аутов

### **Additional Notes:**
Аналогичные ограничения нужно применить к `create_quiz_results_prompt()`.
...
1. В `process_message()` через `_format_text_for_telegram()`
2. В `handle_fragrance_info()` снова через `_format_text_for_telegram()`
