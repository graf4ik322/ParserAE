...
2. **Добавить валидацию URL в `process_ai_response_with_links()`:**
   ```python
...
3. **Проверить запуск планировщика в `main.py`:**
   ```python
...
### **Additional Notes:**
Это критическая проблема для обновления каталога парфюмов. Без работающего парсера каталог не будет обновляться, что приведет к устаревшим данным.